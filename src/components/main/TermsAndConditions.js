...
const TermsAndConditions = (props) => (
	<Row>
		<Col xs={8} xsOffset={2} style={{overflowY: "auto", height: "400px", textAlign: "left", marginTop: "80px"}}>
			<h4 style={{color: "#FF1FA9", marginBottom: "25px", fontSize: "1.1em"}}>Wrklab Terms and Conditions</h4>
			{ 
				props.conditions.map((condition, index) => (
					<div key={index} style={{textAlign: "left", marginBottom: "20px"}}>
						<h4 style={{fontSize: "1.1em", marginBottom: 0}}>{condition.title}</h4>
						<p style={{color: "rgba(0,0,0,.4)", paddingTop: 0}}>{condition.text}</p>
					</div>
				))
			}
		</Col>
